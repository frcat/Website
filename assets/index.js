    ...
    .then((data) => {
        if (!data.members.length == 0) {
            ...
            if (data.members[0].status == "online") {
                document.getElementById("userAvatar").style = "border: 5px solid #3BA55D";
            } else if (data.members[0].status == "idle") {
                document.getElementById("userAvatar").style = "border: 5px solid #FAA81A";
            } else if (data.members[0].status == "dnd") {
                document.getElementById("userAvatar").style = "border: 5px solid #ED4245";
            }
        } else {
...
// Quote
fetch("https://quotes.French-Cat.repl.co/raw")
    .then((res) => res.text())
    .then((quote) => (document.getElementById("quote").innerText = quote));
